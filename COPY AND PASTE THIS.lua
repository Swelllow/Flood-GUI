...
--anticheat bypass all credits to altlexon // 2024 update thank you this will work without custom funcs
local ReplicatedStorage = game:GetService("ReplicatedStorage")

ReplicatedStorage.Remote.ReqCharVars.OnClientInvoke = function()return wait(9e9)end
local FetchPos = ReplicatedStorage.Remote:FindFirstChild("FetchPos")
if FetchPos then
	FetchPos.OnClientInvoke = function()return wait(9e9)end
end
Alert("Anticheat Bypassed!")

local LocalPlayer = game.Players.LocalPlayer
local char = LocalPlayer.Character or (LocalPlayer.CharacterAdded:wait() and LocalPlayer.Character)
local key = -ReplicatedStorage.Remote.ReqPasskey:InvokeServer()
local Alert = function(...)
    ReplicatedStorage.Remote.Alert:FireServer(key, ...)
end
Alert("Flood GUI Zero Loaded!")
...
local Library = (function()
	...
	function Kavo:ToggleUI()
		if LocalPlayer.PlayerGui[LibName].Enabled then
			LocalPlayer.PlayerGui[LibName].Enabled = false
		else
			LocalPlayer.PlayerGui[LibName].Enabled = true
		end
	end
...
local Window = Library.CreateLib("Flood GUI Zero", colors)
local l = Window:NewTab("Movement")
local b = Window:NewTab("Manipulation")
local k = Window:NewTab("Other")
local c = Window:NewTab("Credits")

local loc = l:NewSection("Movement")
local blatant = b:NewSection("Manipulation")
...
other:NewButton("Copy Tomato Support Server invite. [BROKEN]", "Copies Tomato Support Server invite to clipboard!", function()

end)
other:NewButton("Copy Lexon Hub Invite [BROKEN]", "Copies Lexon-Hub invite to clipboard!", function()
...
blatant:NewKeybind("Infinite Air", "Infinite Air/God Mode", Enum.KeyCode.R, function()
	...
	Alert("Godmode toggled to: "..tostring(_G.godmode))
end)

task.spawn(function()
	while task.wait() do
		if _G.godmode == true then
			char:WaitForChild("Humanoid").Health = 99999
		end
	end
end)
...
blatant:NewKeybind("Noclip", "Walk through some walls!", Enum.KeyCode.G, function()
	LocalPlayer.Character:WaitForChild("Torso").CanCollide = _G.noclip
	LocalPlayer.Character:WaitForChild("HumanoidRootPart").CanCollide = _G.noclip
	LocalPlayer.Character:WaitForChild("BoundingBox").CanCollide = _G.noclip
	_G.noclip = not _G.noclip
	Alert("Noclip toggled to: "..tostring(_G.noclip))
end)

_G.infjump = false
blatant:NewKeybind("Infinite Jump", "Jump while freefalling!", Enum.KeyCode.M, function(t)
	infjump = not infjump
	Alert("Infjump toggled to: "..tostring(infjump))
end)
